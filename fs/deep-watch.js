...
if (process.mainModule === module) {
  ...
  // Don't watch logs or leading-dot directories.
  ignoreDir = options.ignoreDir || ignoreDir

  // Only call fs.watch on up to 100 files.
  maxFsWatches = options.maxFsWatches || 1e2

  // Only set up 10K paths for periodic checks.
  maxListSize = options.maxListSize || 1e4

  // Wait 1 second before starting watches.
  fsWatchDelay = options.fsWatchDelay || 1e3

  // Check a modified date every 100ms.
  checkInterval = options.checkInterval || 1e2

  // Don't notify more than once a second.
  notifyInterval = options.notifyInterval || 1e3

  // Remember when it's ok to notify.
  okToNotifyAfter = Date.now() + notifyInterval

  // Don't send changes for swap files, etc.
  ignoreFile = options.ignoreFile || ignoreFile
